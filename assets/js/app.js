...
$.getJSON(
  "https://arcgis.dvrpc.org/portal/rest/services/Boundaries/CountyBoundaries/FeatureServer/0/query?where=DVRPC_REG%3D%27Yes%27&outFields=STATE%2C+CO_NAME&outSR=4326&f=geojson",
  function (data) {
    DVRPC.addData(data);
  }
);
...
var circuit = L.geoJson(null, {
  style: function (feature) {
    switch (feature.properties.circuit) {
      case "Existing":
        return { color: "#8dc63f", weight: 3, opacity: 1 };
      case "Planned":
        return { color: "#329aa7", weight: 3, opacity: 1 };
      case "Pipeline":
        return { color: "#AF46A4", weight: 3, opacity: 1 };
      case "In Progress":
        return { color: "#fdae61", weight: 3, opacity: 1 };
    }
  },
  onEachFeature: function (feature, layer) {
    layer.bindPopup(
      feature.properties.main_trail +
        "<br><i>" +
...
var stations = L.geoJson(null, {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      ...
      if (layer.feature.properties.PED1 > 0) {
        var PEDM = numeral(layer.feature.properties.PED1).format("0,0");
      } else {
        var PEDM = "N/A";
      }

      if (layer.feature.properties.BIKE1 > 0) {
        var BIKEM = numeral(layer.feature.properties.BIKE1).format("0,0");
      } else {
        var BIKEM = "N/A";
      }

      if (layer.feature.properties.PED_YR > 0) {
        var PEDYR = numeral(layer.feature.properties.PED_YR).format("0,0");
      } else {
        var PEDYR = "N/A";
      }

      if (layer.feature.properties.BIKE_YR > 0) {
        var BIKEYR = numeral(layer.feature.properties.BIKE_YR).format("0,0");
      } else {
        var BIKEYR = "N/A";
      }
      ...
      if (trail_id.includes(layer.feature.id)) {
        var TCT =
          '<img class="TCT-tag" src="https://www.dvrpc.org/webmaps/TheCircuit/img/logo_stacked.png" alt="The Circuit Trails"/>';
      } else {
        var TCT = "";
      }
      ...
      $("#feature-list tbody").append(
        '<tr class="feature-row" id="' +
          ...
          '<td class="table-ped" style="vertical-align: middle;text-align:center">' +
          BIKEM +
          "</td>" +
...
attributionControl.onAdd = function (map) {
  ...
  div.innerHTML =
    "<span class='hidden-xs'></span><a href='#' onclick='$(\"#attributionModal\").modal(\"show\"); return false;'>Attribution</a>";
  return div;
};
...
function identify(e) {
  ...
  var content3 = "<div>Pedestrian Volume by Month" + "</div>";

  var content4 = "<div>Bicycle Volume by Month" + "</div>";
  var content5 =
    "<div>Station Information for " + props.LOCATIONNAME + "</div>";
